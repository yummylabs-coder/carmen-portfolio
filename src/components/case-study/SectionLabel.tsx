 ...
 */
export function SectionLabel({ label, accentColor, centered }: SectionLabelProps) {
  if (!label) return null;

  return (
    <span
      className={`inline-flex w-fit rounded-md border px-[10px] py-1 text-11 font-semibold uppercase tracking-[0.05em]${centered ? " mx-auto block" : ""}`}
      style={{
